...
and Splunk. You can use these Terraform modules as a baseline and will need to customise the necessary roles, permissions, 
keys, identifications etc. to suit your environment. The steps to configure the plugins and various settings on Microsoft
Sentinel and Splunk themselves are also specific to your environment. These steps are not included in the repository 
but can be found online. Two useful links are listed below.
...
Terraform modules ec2 and vpc are only to run a temporary instance of Splunk Enterprise on a VM and are not needed 
for production deployment (unless that is what your production environment is going to be.)
...
access control lists and encryption however (if properly setup and the keys have not been stolen) will still act as 

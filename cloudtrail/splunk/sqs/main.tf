...
resource "aws_sqs_queue" "cloudtrail_splunk_queue" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = aws_sqs_queue.cloudtrail_splunk_queue_deadletter.arn
    maxReceiveCount = 4
  })
}

resource "aws_sqs_queue_redrive_allow_policy" "cloudtrail_splunk_queue_redrive_allow_policy" {
  queue_url = aws_sqs_queue.cloudtrail_splunk_queue_deadletter.id

  redrive_allow_policy = jsonencode({
    redrivePermission = "byQueue",
    sourceQueueArns = [aws_sqs_queue.cloudtrail_splunk_queue.arn]
  })
}

resource "aws_sqs_queue" "cloudtrail_splunk_queue_deadletter" {
  name = "cloudtrail-splunk-deadletter-queue"
}

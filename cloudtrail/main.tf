...
data "aws_iam_policy_document" "cloudtrail_bucket_policy" {
 ...
 # Allow Firehose to put and get failed deliveries
  statement {
    sid = "AllowFederatedSentinelRoleCloudTrailFailedLogfiles"
    effect = "Allow"
    ...
    # Fine tune this action later to read and write only
    actions = ["s3:*"]
    resources = [
      "${aws_s3_bucket.cloudtrail_bucket.arn}/firehose_failed_deliveries",
      "${aws_s3_bucket.cloudtrail_bucket.arn}/firehose_failed_deliveries/*"
    ]
  }
